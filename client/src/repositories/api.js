...
export const putAPI = async (params, data, token) => {
  const res = await axiosClient.put(`/${params}`, data, { headers: { Authorization: `Bearer ${token}` } });
  return res.data;
};

export const deleteAPI = async (params, token) => {
  const res = await axios.get(`${baseUrl}/${params}`, { headers: { Authorization: `Bearer ${token}` } });
  return res.data;
};
